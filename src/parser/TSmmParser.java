...
public class TSmmParser extends Parser {
	...
	public final RecordFieldsContext recordFields() throws RecognitionException {
		...
		try {
			...
			match(T__2);
			setState(234);
			((RecordFieldsContext)_localctx).t1 = type();
			setState(235);
			match(T__3);
			_localctx.ast.add(new RecordField((((RecordFieldsContext)_localctx).ID1!=null?((RecordFieldsContext)_localctx).ID1.getText():null), ((RecordFieldsContext)_localctx).t1.ast));
			}
		}
		catch (RecognitionException re) {
			_localctx.exception = re;
